...
/* Core payoff engine */
function calculatePayoff(debts: Debt[], strategy: Strategy, extraPayment: number) {
  if (debts.length === 0)
  ...
  const totalBudget = totalMinimum + extraPayment;
  ...
  chartData.push({ month: "Now", balance: Math.round(startBalance), paid: 0 });
  ...
  while (Object.values(balances).some((b) => b > 0) && month < 600) {
    ...
    const currentBalance = Object.values(balances).reduce((s, b) => s + Math.max(0, b), 0);
  ...
  return { months: month, totalInterest: Math.round(totalInterest), chartData, debtFreeDate: payoffDate(month) };
}
            ...
            <div>
              ...
              <p style={{ color: "var(--text-muted)", fontSize: "0.875rem", margin: "0.25rem 0 0" }}>
                {debts.length === 0
                  ...
                  : `Tracking ${debts.length} debt${debts.length > 1 ? "s" : ""} · ${formatCurrency(totalBalance)} total`}
              </p>
            </div>
            ...
            {(["overview", "debts", "plan"] as const).map((tab) => (
              <button
                ...
                style={{
                  ...
                  border: activeTab === tab ? "1px solid rgba(0,201,167,0.3)" : "1px solid transparent",
                  cursor: "pointer",
                  fontSize: "0.875rem",
                  fontWeight: 500,
                  background: activeTab === tab ? "rgba(0,201,167,0.1)" : "transparent",
                  color: activeTab === tab ? "var(--accent-mint)" : "var(--text-muted)",
      ...
      <div className="section-wrapper" style={{ padding: "2rem 1.5rem", position: "relative", zIndex: 1 }}>
        ...
        {activeTab === "overview" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "1rem" }}>
              <SummaryCard label="Total debt"        value={formatCurrency(totalBalance)}         sub={`${debts.length} account${debts.length !== 1 ? "s" : ""}`} color="var(--accent-ember)" accent="none" />
              <SummaryCard label="Monthly minimum"   value={formatCurrency(totalMinimum)}         sub="required payments"                     color="var(--accent-gold)"  accent="none" />
              <SummaryCard label="Debt-free date"    value={activeResult.debtFreeDate}            sub={formatMonths(activeResult.months) + " away"} color="var(--accent-mint)"  accent="mint" />
              <SummaryCard label="Interest you'll pay" value={formatCurrency(activeResult.totalInterest)} sub={strategy + " strategy"}         color="var(--text-muted)"   accent="none" />
            </div>

            {/* Strategy selector + extra payment */}
            <GlassCard variant="default" radius="rounded" className="p-6">
              <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", alignItems: "flex-start" }}>
                    ...
                    {(["avalanche", "snowball"] as Strategy[]).map((s) => (
                      <button
                        ...
                        style={{
                          ...
                          border: `1px solid ${strategy === s ? "var(--accent-mint)" : "rgba(141,160,179,0.2)"}`,
                          background: strategy === s ? "rgba(0,201,167,0.12)" : "rgba(255,255,255,0.04)",
                ...
                <div style={{ flex: "1 1 220px" }}>
                  <div className="stat-label" style={{ marginBottom: "0.75rem" }}>
                    ...
                    <span style={{ color: "var(--accent-mint)" }}>{formatCurrency(extraPayment)}</span>
                  </div>
                  <input
                    type="range" min={0} max={2000} step={25} value={extraPayment}
                ...
                {avalancheResult.months > 0 && snowballResult.months > 0 && (
                  <div style={{
                    ...
                    <CompareRow label="Avalanche" months={avalancheResult.months} interest={avalancheResult.totalInterest} active={strategy === "avalanche"} color="var(--accent-mint)" />
                    <div style={{ height: "0.5rem" }} />
                    <CompareRow label="Snowball"  months={snowballResult.months}  interest={snowballResult.totalInterest}  active={strategy === "snowball"}  color="var(--accent-gold)" />
                  </div>
                )}
              </div>
            </GlassCard>
            ...
            {debts.length > 0 && activeResult.chartData.length > 1 && (
              <GlassCard variant="default" accent="mint" radius="rounded" className="p-6">
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <AreaChart data={activeResult.chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip
                      contentStyle={{ background: "rgba(28,46,64,0.92)", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "10px", fontSize: "0.8rem" }}
            ...
            {/* Debt breakdown */}
            {debts.length > 0 && (
              <GlassCard variant="subtle" radius="rounded" className="p-6">
        ...
        {activeTab === "debts" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            {showForm && (
              <GlassCard variant="elevated" accent="mint" radius="rounded" className="p-6">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem" }}>
                  <FormField label="Account name" error={formErrors.name}>
                    <input type="text" placeholder="e.g. Chase Freedom" value={form.name}
                      onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} style={inputStyle(!!formErrors.name)} />
                  </FormField>
                  <FormField label="Debt type" error={""}>
                    <select value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value }))} style={inputStyle(false)}>
                      {DEBT_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                    </select>
                  </FormField>
                  <FormField label="Current balance ($)" error={formErrors.balance}>
                    <input type="number" placeholder="e.g. 4800" value={form.balance} min={0}
                      onChange={(e) => setForm((f) => ({ ...f, balance: e.target.value }))} style={inputStyle(!!formErrors.balance)} />
                  </FormField>
                  <FormField label="Interest rate (APR %)" error={formErrors.interestRate}>
                    <input type="number" placeholder="e.g. 21.99" value={form.interestRate} min={0} step={0.01}
                      onChange={(e) => setForm((f) => ({ ...f, interestRate: e.target.value }))} style={inputStyle(!!formErrors.interestRate)} />
                  </FormField>
                  <FormField label="Minimum monthly payment ($)" error={formErrors.minimumPayment}>
                    <input type="number" placeholder="e.g. 96" value={form.minimumPayment} min={0}
                      onChange={(e) => setForm((f) => ({ ...f, minimumPayment: e.target.value }))} style={inputStyle(!!formErrors.minimumPayment)} />
                  </FormField>
                </div>
                <div style={{ display: "flex", gap: "0.75rem", marginTop: "1.25rem" }}>
                  <button className="btn-primary" onClick={handleSubmit}>{editingId ? "Save changes" : "Add debt"}</button>
                  <button className="btn-secondary" onClick={handleCancel}>Cancel</button>
                </div>
              </GlassCard>
            )}

            {debts.length === 0 && !showForm && <EmptyState onAdd={() => setShowForm(true)} />}
                ...
                >
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap" }}>
                    ...
                    <div style={{ display: "flex", gap: "0.5rem", flexShrink: 0 }}>
                      <button onClick={() => handleEdit(debt)}   style={ghostBtnStyle}>Edit</button>
                      <button onClick={() => handleDelete(debt.id)} style={{ ...ghostBtnStyle, color: "var(--accent-ember)" }}>Remove</button>
                    </div>
                  </div>
                </GlassCard>
              );
            ...
            {debts.length > 0 && !showForm && (
              <button className="btn-secondary" onClick={() => setShowForm(true)} style={{ alignSelf: "flex-start" }}>
                + Add another debt
              </button>
            )}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            {debts.length === 0 ? (
              ...
              <>
                {/* Plan cards */}
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem" }}>
                  <PlanCard strategy="avalanche" result={avalancheResult} active={strategy === "avalanche"} onSelect={() => setStrategy("avalanche")} description="Highest interest first. Mathematically optimal — saves the most in interest." />
                  <PlanCard strategy="snowball"  result={snowballResult}  active={strategy === "snowball"}  onSelect={() => setStrategy("snowball")}  description="Smallest balance first. Builds momentum with early wins." />
                </div>

                {/* Interest savings callout */}
                {avalancheResult.totalInterest !== snowballResult.totalInterest && (
                  <GlassCard variant="subtle" accent="mint" radius="rounded" className="px-5 py-4">
                    ...
                    .map((debt, i) => {
                      ...
                      return (
                        <div key={debt.id} style={{
                          ...
                          padding: "0.875rem 0",
                          borderBottom: i < debts.length - 1 ? "1px solid rgba(255,255,255,0.06)" : "none",
                        }}>
                          <span style={{
                            width: 28, height: 28, borderRadius: "50%",
                            background: i === 0 ? "var(--accent-mint)" : "rgba(255,255,255,0.06)",
                            color: i === 0 ? "#0D1B2A" : "var(--text-muted)",
                            display: "flex", alignItems: "center", justifyContent: "center",
                            fontSize: "0.75rem", fontWeight: 700, flexShrink: 0,
                            boxShadow: i === 0 ? "0 0 12px rgba(0,201,167,0.4)" : "none",
                          }}>{i + 1}</span>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontWeight: 600, fontSize: "0.9rem" }}>{debt.name}</div>
                            <div style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                              {formatCurrency(debt.balance)} · {(debt.interestRate * 100).toFixed(2)}% APR
                            </div>
                          </div>
                          <div style={{ textAlign: "right", flexShrink: 0 }}>
                            ...
                            {i === 0 && (
                              <span style={{ fontSize: "0.7rem", color: "var(--accent-mint)", fontWeight: 600 }}>← Focus here</span>
                            )}
                          </div>
                        </div>
                      );
                ...
                {/* Educational note */}
                <GlassCard variant="subtle" radius="rounded" className="p-5">
...
function SummaryCard({ label, value, sub, color, accent }: {
  ...
  accent: "none" | "mint" | "blue" | "purple" | "gold" | "pink" | "teal";
}) {
  return (
    <GlassCard
      variant="default"
...
function CompareRow({ label, months, interest, active, color }: {
  label: string; months: number; interest: number; active: boolean; color: string;
}) {
  return (
...
function PlanCard({ strategy, result, active, onSelect, description }: {
  ...
  const isMint = strategy === "avalanche";
  const color  = isMint ? "var(--accent-mint)" : "var(--accent-gold)";
  const accent = isMint ? "mint" : "gold";

  return (
    <GlassCard
      variant={active ? "elevated" : "subtle"}
      accent={active ? accent : "none"}
      radius="rounded"
      interactive
      glow={active && isMint}
      onClick={onSelect}
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <GlassCard variant="subtle" radius="soft" className="py-16 px-8" style={{ textAlign: "center", border: "1px dashed rgba(141,160,179,0.2)" }}>
...
const inputStyle = (hasError: boolean): React.CSSProperties => ({
  ...
  border: `1px solid ${hasError ? "var(--accent-ember)" : "rgba(255,255,255,0.1)"}`,
  borderRadius: "var(--radius-sm)",
